...
class LiDAR_Lite():
    ...
    def get_distance(self):
        ...
        result = self.read_reg2("FULL_DELAY")
        return result / 25.6  # dividing by 25.6 convert to mm.
    ...
    def read_reg(self, reg):
        ...
        """
        return self.bus.readfrom_mem(self.addr, self.reg[reg], 1)
    ...
    def read_reg2(self, reg):
        ...
        """
        high_byte = self.bus.readfrom_mem(self.addr, self.reg[reg], 1)
        low_byte = self.bus.readfrom_mem(self.addr, self.reg[reg] + 1, 1)
        return unpack("<H", bytearray(high_byte + low_byte))[0] # (high_byte << 8) + low_byte
    ...
    def write_reg(self, reg, val):
        ...
        """
        self.bus.writeto_mem(self.addr, self.reg[reg], val)
    ...
    def write_reg2(self, reg, val):
        ...
        """
        self.bus.writeto_mem(self.addr, self.reg[reg], val >> 8)
        self.bus.writeto_mem(self.addr, self.reg[reg] + 1, val & 0b11111111)
